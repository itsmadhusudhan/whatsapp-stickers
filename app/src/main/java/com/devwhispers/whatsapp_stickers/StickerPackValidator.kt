...
internal object StickerPackValidator {
    ...
    // using this instead of itunes domain for now
    private const val APPLE_STORE_DOMAIN = "apps.apple.com"
    ...
    fun verifyStickerPackValidity(context: Context, stickerPack: StickerPack) {
        ...
        if (stickerPack.publisherEmail?.isEmpty() == true && !Patterns.EMAIL_ADDRESS.matcher(
        ...
        for (sticker in stickers) {
            validateSticker(
                context,
                stickerPack.identifier, sticker, stickerPack.animatedStickerPack
            )
        }
    }
    ...
    private fun validateSticker(
        ...
        validateStickerFile(
            context, identifier,
            sticker.imageFileName!!, animatedStickerPack
        )
    }
    ...
    private fun validateStickerFile(
        ...
        try {
            ...
            try {
                val webPImage: WebPImage =
                    WebPImage.createFromByteArray(stickerInBytes, ImageDecodeOptions.defaults())
                if (webPImage.height!== IMAGE_HEIGHT) {
                    throw IllegalStateException(("sticker height should be " + IMAGE_HEIGHT + ", current height is " + webPImage.getHeight()) + ", sticker pack identifier: " + identifier + ", filename: " + fileName)
                }
                if (webPImage.width !== IMAGE_WIDTH) {
                    throw IllegalStateException(("sticker width should be " + IMAGE_WIDTH + ", current width is " + webPImage.getWidth()) + ", sticker pack identifier: " + identifier + ", filename: " + fileName)
                }
                if (animatedStickerPack) {
                    ...
                    if (webPImage.duration > ANIMATED_STICKER_TOTAL_DURATION_MAX) {
                        throw IllegalStateException(("sticker animation max duration is: " + ANIMATED_STICKER_TOTAL_DURATION_MAX + " ms, current duration is: " + webPImage.getDuration()) + " ms, sticker pack identifier: " + identifier + ", filename: " + fileName)
                    }
