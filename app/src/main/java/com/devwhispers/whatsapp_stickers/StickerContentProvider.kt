...
class StickerContentProvider : ContentProvider() {
    ...
    override fun onCreate(): Boolean {
        ...
        getStickerPackList()?.forEach {
            MATCHER.addURI(
                authority,
                STICKERS_ASSET + "/" + it.identifier + "/" + it.trayImageFile,
                STICKER_PACK_TRAY_ICON_CODE
            )
            it.stickers.forEach { sticker ->
                MATCHER.addURI(
                    authority,
                    STICKERS_ASSET + "/" + it.identifier + "/" + sticker.imageFileName,
                    STICKERS_ASSET_CODE
                )
            }
        }
        ...
        getStickerPackList()?.forEach { stickerPack ->
            if (identifier == stickerPack.identifier) {
                stickerPack.stickers.forEach { sticker ->
                    cursor.addRow(
                        arrayOf(sticker.imageFileName!!, TextUtils.join(",", sticker.emojis!!))
                    )
                }
            }
        }
        ...
        for (stickerPack in getStickerPackList()!!) {
            if (identifier == stickerPack.identifier) {
                if (fileName == stickerPack.trayImageFile) {
                    ...
                    stickerPack.stickers.let { stickerList ->
